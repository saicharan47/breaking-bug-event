...
export async function saveTeamAnswer(teamCode:string,questionId:number,answer:string,runId?:string){const db=await getFirebaseDatabase();if(!db)throw new Error('Firebase database is unavailable.');try{await update(ref(db,teamPath(teamCode)),{[`answers/${questionId}`]:answer,...(runId?{runId}:{})});}catch(error){throw firebaseError(`Saving answer for ${teamCode}`,error);}}
