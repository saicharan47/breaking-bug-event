...
export async function getRoundConfig(round=1){ const db=await getFirebaseDatabase(); if(!db)return DEFAULT_ROUND1_CONFIG; try { const stored=(await get(ref(db,`rounds/${round}/config`))).val() as Partial<RoundConfig>|null; return {...DEFAULT_ROUND1_CONFIG,...stored}; } catch { return DEFAULT_ROUND1_CONFIG; } }
...
export async function saveRoundConfig(round:number, config:RoundConfig){
  const db=await getFirebaseDatabase(); if(!db)throw new Error('Firebase database is unavailable.');
  if(!Number.isFinite(config.durationSeconds)||config.durationSeconds<=0||!Number.isFinite(config.questionDurationSeconds)||config.questionDurationSeconds<=0||!Number.isInteger(config.questionCount)||config.questionCount<1||!Number.isInteger(config.advanceCount)||config.advanceCount<1||config.advanceCount>config.questionCount)throw new Error('Round 1 timer or advancement configuration is invalid.');
